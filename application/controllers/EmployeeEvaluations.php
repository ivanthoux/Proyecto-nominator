...
class EmployeeEvaluations extends ManagerController
{

  protected $pathindex = '/employeeevaluations/all';
  protected $viewpath = 'manager/employeeevaluations/employeeevaluation';
  ...
  public function all()
  {
    $this->load->model(['EmployeeEvaluation_model']);
    $employeeEvaluationModel = new EmployeeEvaluation_model();
    // $this->data['filters'] = $employeeEvaluationModel->get_filters();
    $this->data['filters'] = [];
    $viewpath = $this->viewpath . '_list';

    $this->data['activesidebar'] = 'employeeevaluations';
  ...
  {
    $this->load->model(['EmployeeEvaluation_model', 'Persons_model']);
    $employeeEvaluationModel = new EmployeeEvaluation_model();
    ...
    if (!empty($post)) { //saving user received
      ...
      if (empty($this->data['errors'])) {
        $employeeEvaluationDate = DateTime::createFromFormat('d-m-Y', $post['employeeevaluation_date']);
        $post['employeeevaluation_date'] = $employeeEvaluationDate->format('Y-m-d');
        $personId = $post['employeeevaluation_person'];
        $id = $employeeEvaluationModel->save($post);
  ...
  public function datatables()
  {
    $this->load->model('EmployeeEvaluation_model');
    $employeeEvaluationModel = new EmployeeEvaluation_model();
    echo json_encode($employeeEvaluationModel->datatables_ajax_list());
  }
  ...
  {
    if (empty($post['employeeevaluation_person'])){
      $this->data['errors'][] = '<p>Persona inexistente.</b></p>';  
    }
    if (empty($post['employeeevaluation_date'])){
      $this->data['errors'][] = '<p>Debe ingresar una fecha.</b></p>';  
    }
  }
