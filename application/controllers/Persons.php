...
class Persons extends ManagerController
{

  protected $pathindex = '/persons/all';
  protected $viewpath = 'manager/persons/persons';
  ...
  public function all()
  {
    ...
    $persons_model = new Persons_model();

    $viewpath = $this->viewpath . '_list';

    $this->data['filters'] = $persons_model->get_filters();
  ...
  {
    $this->load->model(['Persons_model', 'Regions_model', 'Cities_model']);
    $persons_model = new Persons_model();
    ...
    if (!empty($post)) { //saving user received
      $thereIsPersonBirth = !empty($post['person_birth']);
      if ($thereIsPersonBirth) {
        $personBirthDateObject = DateTime::createFromFormat('d-m-Y', $post['person_birth']);
        $post['person_birth'] = $personBirthDateObject->format('Y-m-d');
      }
      $this->_checkConstrain($post);
      if (empty($this->data['errors'])) {
        $id = $persons_model->save($post);
        redirect('/persons/all', 'refresh');
      }
    }
    if (!empty($_id)) {
      $this->data['edit'] = $persons_model->get_by_id($_id);
    }
  ...
  public function datatables()
  {
    $this->load->model('Persons_model');
    $persons_model = new Persons_model();
    echo json_encode($persons_model->datatables_ajax_list());
  }
  ...
  private function _checkConstrain($person)
  {
    $personsModel = new Persons_model();
    if (empty(trim($person['person_doc']))) {
      $this->data['errors'][] = '<p>Formato de Código de persona invalido</p>';
    } else {
      $c = $personsModel->get_by_doc($person['person_doc']);
      if (!empty($c)) {
        if ($c['person_id'] != $person['person_id']) {
          $this->data['errors'][] = '<p>Ya existe un persona con este número de persona: <b>' . $c['personhistory_firstname'] . ' ' . $c['personhistory_lastname'] . '</b></p>';
        }
      }
    }
    if (!empty($person['person_email']) && !filter_var($person['person_email'], FILTER_VALIDATE_EMAIL)) {
