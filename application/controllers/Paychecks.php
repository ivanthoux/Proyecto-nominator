...
class Paychecks extends ManagerController
{

  protected $pathindex = '/paychecks/all';
  protected $viewpath = 'manager/paychecks/paycheck';
  ...
  public function all()
  {
    ...
    $this->data['activesidebar'] = 'paychecks';
  ...
  {
    $this->load->model(['Paycheck_model', 'Contract_model', 'Persons_model', 'Additionals_model', 'Paycheck_additionals_model']);
    $paycheckAdditionalsModel = new Paycheck_additionals_model();
    $paycheckModel = new Paycheck_model();
    $contractModel = new Contract_model();
    $personsModel = new Persons_model();
    $additionalsModel = new Additionals_model();

    $viewpath = $this->viewpath . '_form';
    $this->data['additionals'] = $additionalsModel->get_all();
    $now = new DateTimeImmutable();
    $now = $now->format('Y-m-d');
    $this->data['contract'] = $contractModel->getByPersonAndDate($personId, $now);
    ...
    if (!empty($post)) { //saving user received
      ...
      if (empty($this->data['errors'])) {
        ...
        foreach ($post as $key => $value) {
          ...
          if (strpos($key, "paycheck_additional_") !== false) {
            if (!empty($value)) {
              $newKey = str_replace("paycheck_additional_", '', $key);
              $newKey = str_replace("_single", '', $newKey);
              $additionals[$newKey] = $value;
            }
            unset($post[$key]);
          }
        }
        $paycheckStart = DateTime::createFromFormat('d-m-Y', $post['paycheck_date']);
        $post['paycheck_date'] = $paycheckStart->format('Y-m-d');
        $personId = $post['paycheck_person'];
        // $this->dd($additionals);
        $payCheckId = $paycheckModel->save($post);
        foreach($additionals as $key => $additional){
          $fullAdditional = $additionalsModel->getAdditionalByKey($key);
          $additionalId = $fullAdditional['additional_id'];
          $additionalOrder = $fullAdditional['additional_order'];
          $additionalCoefficient = $fullAdditional['additional_coefficient'];
          $additionalBase = $additional / $additionalCoefficient;
          $additionalRemunerative = $fullAdditional['additional_remunerative'];
          $additionalHaber = $fullAdditional['additional_haber'];
          $paycheckAdditionalsModel->save([
            "paycheckadditional_paycheck" => $payCheckId,
            "paycheckadditional_additional" => $additionalId,
            "paycheckadditional_order" => $additionalOrder,
            "paycheckadditional_base" => $additionalBase,
            // "paycheckadditional_units" => $additionalBase,
            "paycheckadditional_coefficient" => $additionalCoefficient,
            "paycheckadditional_remunerative" => $additionalRemunerative,
            "paycheckadditional_haber" => $additionalHaber,
          ]);
        }
  ...
  public function remove($paycheck_id)
  {
    $this->load->model('Contract_model');
    $this->Contract_model->delete($paycheck_id);
  ...
  public function datatables()
  {
    ...
    $payChecktModel = new Paycheck_model();
    echo json_encode($payChecktModel->datatables_ajax_list());
  }
  ...
  {
    if (empty($post['paycheck_date'])) {
      $this->data['errors'][] = '<p>Debe ingresar una fecha de liquidación de sueldo.</b></p>';
    }
  }
